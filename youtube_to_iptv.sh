...
for channel in "${!channels[@]}"; do
    ...
    if [ -n "$stream_url" ]; then
        echo "#EXTINF:-1 tvg-id=\"$channel\" tvg-logo=\"$logo_url\" group-title=\"YouTube\",$channel" >> youtube_iptv.m3u
        echo "$stream_url" >> youtube_iptv.m3u
    else
